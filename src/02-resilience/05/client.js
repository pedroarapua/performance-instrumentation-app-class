...
breaker.on('open', async () => {
  console.info(`OPEN: The breaker`);

  try {
    await produceMessage('CB get shipping is open');
  } catch(err) {
    console.error('Error to produce message in Rabbitmq => ', err);
  }
});
...
// function to produce message in rabbitmq
